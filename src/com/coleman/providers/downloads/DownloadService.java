...
public class DownloadService extends Service {
        ...
        public void run() {
            ...
            for (;;) {
                ...
                Cursor cursor = getContentResolver().query(
                        Downloads.Impl.ALL_DOWNLOADS_CONTENT_URI, null, null, null, null);
                if (cursor == null) {
                    continue;
                }
                try {
                    DownloadInfo.Reader reader = new DownloadInfo.Reader(getContentResolver(),
                            cursor);
                ...
                for (DownloadInfo info : mDownloads.values()) {
                    if (info.mDeleted) {
                        ...
                        if (TextUtils.isEmpty(info.mMediaProviderUri)) {
                            ...
                            // throw new
                            // IllegalStateException("scanFile failed!");
                            // }
                            // } else {
    ...
    // // so that it is NOT subject to re-scanning
    // // by MediaScanner
    // // next time this database row is
    // // encountered
    ...
    // } else if (deleteFile) {
    // if (uri != null) {
    // // use the Uri returned to delete it
    // // from the MediaProvider
    // getContentResolver().delete(uri, null, null);
    // }
    // // delete the file and delete its row from
    // // the downloads db
