...
public class Helpers {
    ...
    public static String generateSaveFile(Context context, String url, String hint,
            ...
            long contentLength, boolean isPublicApi) throws GenerateSaveFileError {
        checkCanHandleDownload(context, mimeType, destination, isPublicApi);
        if (destination == Downloads.Impl.DESTINATION_FILE_URI) {
            return getPathForFileUri(hint, contentLength);
        } else {
            return chooseFullPath(context, url, hint, contentDisposition, contentLocation,
                    mimeType, destination, contentLength);
        }
    }
    ...
    private static String chooseUniqueFilename(int destination, String filename, String extension,
            boolean recoveryDir) throws GenerateSaveFileError {
    ...
    public static final boolean discardPurgeableFiles(Context context, long targetBytes) {
        Cursor cursor = context.getContentResolver().query(
                ...
                "( " + Downloads.Impl.COLUMN_STATUS + " = '" + Downloads.Impl.STATUS_SUCCESS
                        + "' AND " + Downloads.Impl.COLUMN_DESTINATION + " = '"
                        + Downloads.Impl.DESTINATION_CACHE_PARTITION_PURGEABLE + "' )", null,
        ...
        try {
            ...
            while (!cursor.isAfterLast() && totalFreed < targetBytes) {
                ...
                if (Constants.LOGVV) {
                    Log.v(Constants.TAG,
                            "purging " + file.getAbsolutePath() + " for " + file.length()
                                    + " bytes");
                }
        ...
        if (Constants.LOGV) {
            if (totalFreed > 0) {
                Log.v(Constants.TAG, "Purged files, freed " + totalFreed + " for " + targetBytes
                        + " requested");
            }
        }
        return totalFreed > 0;
     ...
     * database.
     */
    /* package */static void deleteFile(ContentResolver resolver, long id, String path,
            String mimeType) {
        try {
